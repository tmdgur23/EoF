	...
	public class ScriptableTextTypeListEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			for (int i = 0; i < m_typeList.arraySize; i++)
			{
				...
				SerializedProperty FixedStartProp =
					listReference.FindPropertyRelative("FixedStartPosition");
					...
					if (m_renderGroupFolded)
					{
						...
						switch (t)
						{
							case ScriptableText.TextRenderMode.ScreenSpace:
								EditorGUILayout.PropertyField(FixedStartProp);
								GUI.enabled = FixedStartProp.boolValue;
