...
namespace MessagePack.Unity
{
	...
	public sealed class SpriteFormatter : global::MessagePack.Formatters.IMessagePackFormatter<global::UnityEngine.Sprite>
	{
		public int Serialize(ref byte[] bytes, int offset, global::UnityEngine.Sprite value, IFormatterResolver typeResolver)
		{
			...
			{
				var result = path.Split(new[] {"Resources/"},StringSplitOptions.None);
				if (path.Length <= 1)
				{
					Debug.LogError("Some File wasn't placed in an Resources Folder!",value);
				}
				path = result[1];
	...
	public sealed class AudioClipFormatter : global::MessagePack.Formatters.IMessagePackFormatter<global::UnityEngine.AudioClip>
	{
		public int Serialize(ref byte[] bytes, int offset, global::UnityEngine.AudioClip value, IFormatterResolver typeResolver)
		{
			var path = "";
		#if UNITY_EDITOR
			path = AssetDatabase.GetAssetPath(value);
		...
		public global::UnityEngine.AudioClip Deserialize(byte[] bytes, int offset, IFormatterResolver typeResolver, out int readSize)
		{
			var path = MessagePackBinary.ReadString(bytes, offset, out readSize);
			var audioClip = Resources.Load<global::UnityEngine.AudioClip>(path);
			return audioClip;
		}
	}
	
	public sealed class ScriptableObjectFormatter : global::MessagePack.Formatters.IMessagePackFormatter<global::UnityEngine.ScriptableObject>
	{
		public int Serialize(ref byte[] bytes, int offset, global::UnityEngine.ScriptableObject value, IFormatterResolver typeResolver)
		{
			var path = "";
		#if UNITY_EDITOR
			path = AssetDatabase.GetAssetPath(value);
		...
		public global::UnityEngine.ScriptableObject Deserialize(byte[] bytes, int offset, IFormatterResolver typeResolver, out int readSize)
		{
			var path = MessagePackBinary.ReadString(bytes, offset, out readSize);
			var gbj = Resources.Load<global::UnityEngine.ScriptableObject>(path);
			return gbj;
		}
	}
}